...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_words_with_more_than_three_letters
    ...
    selected = []
    words.each {|word| selected << word unless word.size > 3}
  ...
  def test_remove_numbers
    ...
    not_numbers = []
    elements.each {|ele| not_numbers << ele unless ele.is_a?(Numeric)}
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each {|ele| remaining << ele unless ele.is_a?(Array)}
  ...
  def test_remove_hashes
    ...
    remaining = []
    elements.each {|ele| remaining << ele unless ele.is_a?(Hash)}
